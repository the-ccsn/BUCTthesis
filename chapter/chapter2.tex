...
\begin{itemize}
	...
	\item 其它选项：多余的选项将传递给 \pkg{ctexbook} 文类，如 \opt{draft} 将关闭插图和部分宏包的渲染，从而加快编译速度。
\end{itemize}
...
而该部分使用 \cmd{taskinfo} 或 \cmd{taskinfo*} 命令来插入。
...
\begin{enumerate}
	...
	宋体和黑体均可加粗为对应的伪粗体；《规范》中未规定的楷体和仿宋均为 \pkg{ctex} 宏集的预设，应减少使用；
...
\begin{longtable}[c]{*{5}{l}r}
	...
	\toprule
	\textbf{Entry} & \textbf{聚合物基体} & \textbf{锂盐} & \textbf{塑化剂} & \textbf{温度 (\si{\degreeCelsius})} & \textbf{离子电导率 (\si{S.cm^{-1}})}\\ \midrule
	\endfirsthead
...
《规范》中未详细规定符号说明部分的格式，模板设计了一个无线可跨页的长表格，
直接在环境里填入内容即可。
环境的接受两个参数\cargu{SymbolWid}和\cargu{DenoWid}，分别代表“符号”和“说明”二列的宽度，用于在必要时调整。
