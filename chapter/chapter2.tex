...
\begin{itemize}
	...
	\item \opt{openany} 或 \opt{openright}：二选一。后者将会设置每章只从奇数页开始，之前不足的页面用完全空白页补足；
...
无论是文档类选项还是 \cmd{buctsetup} 命令，各选项之间不要留有空行，并以西文逗号“,”分隔。

\section{前置部分}\label{sec:frontmatter}
在“目录”之前的几个部分，包括“诚信声明”、“任务书”、“摘要”和“Abstract”，分别对应于 \file{chapter/frontmatter} 中的不同环境。

\subsection{摘要和关键词}\label{subsec:abstract}
中、英文摘要分别使用\env{abstract}和\env{abstracten}环境，或\env{abstract*}和\env{abstracten*}环境来输出。
区别在于：前者会将中英文摘要分排在不同页上；而后者将会使二者尽可能地压缩在一页之中，且可以使用相关命令进一步控制行距。

插入中、英文关键词分别使用 \cmd{keywords\argu{keywords}} 和 \cmd{keywordsen\argu{keywords}}命令，
...
会作用于“任务书”的开头部分，使用 \cmd{taskinfo} 或 \cmd{taskinfo*} 命令来插入。
...
\begin{enumerate}
	...
	文件中的 \cmd{setCJKmainfont}\{SimSun\} 处，将 SimSun 替换为合适的衬线字体；无衬线字体同理。}
	且二者均可加粗为对应的伪粗体；《规范》中未规定的楷体和仿宋均为 \pkg{ctex} 宏集的预设，应减少使用；
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{example/BUCT-logo.pdf}
	\caption{校徽和校名}\label{fig:WholeLogo}
\end{figure}

至于图片的并排，如果只需为组图写一个图注，可在一个 \env{figure} 环境中多次使用 \cmd{includegraphics} 命令（可根据需要在插图之间加入空白）；
但如果需要在每一个子图下写上图注（或需要对子图标序），可使用\env{sub\-figure}环境。
校名见图~\ref{subfig:znname}，校徽见图~\ref{subfig:logo}，校名和校徽见图~\ref{fig:wholelogo}。
\begin{figure}[H]
	\centering%
	\begin{subfigure}[t]{6cm}
		\includegraphics[width=6cm]{BUCT-name.pdf}
		\caption{这是校名。如果这个标题比较长，那么它能自动换行。}\label{subfig:znname}
	\end{subfigure}
	\hspace{1cm}
	\begin{subfigure}[t]{2.5cm}
		\includegraphics[scale=0.4]{BUCT-badge.pdf}
...
\begin{dfigure}% [H] is also available here.
	\centering
	\includegraphics[width = \textwidth]{example/蜗轮.pdf}
...
使用 \cmd{hline}命令也能划线，但其线宽固定。关于表格内对齐与常用的命令见表~\ref{tab:tabcmd}。
\begin{table}[H]
	\centering
...
\cmd{bottomrule}后可以加一个可选参数来实现对线宽的控制，如果不加参数则为默认值；
而\cmd{cline}可针对某些表列画上横线。
...
\begin{itemize}
	...
	\item 在论文中使用 \cmd{cite}命令引用相应的文献。
	% 这里用了一条简单的自定义命令，用于快速插入单张图片，见 \file{myconfig.tex}文件。
	\addfig[12cm]{AddBib.png}{在谷歌学术中导出参考文献的步骤}{fig:addbib}
\end{itemize}
...
直接在环境里填入内容即可。
环境接受两个参数\cargu{SymbolWid}和\cargu{DenoWid}，分别代表“符号”和“说明”二列的宽度，用于在必要时调整。
