%% 第一章--chapter1.tex
\chapter{绪论}\echapter{Introduction}
\section{字体}\esection{Font}
\subsection{字体配置}\esubsection{Font Settings}
模板配置了两种字库，以 \opt{fontset = fandol} 为例：
\begin{enumerate}
...
\subsection{中文字体命令及对应西文示例}\esubsection{Font Commands}
\begin{enumerate}
...
\subsection{插图}\esubsection{Figures}\label{subsec:fig}
...
\begin{figure}[H]
	...
	\bicaption{一张插图。加上参数 [H] 固定图片位置，禁止“浮动”。}{This's a figure. Forbidden floating with the [H] option.}\label{fig:a-single-image}
\end{figure}
...
\begin{figure}[htbp]
	...
	\bicaption{并排的插图，这适合无需在每一张图下写图注的情况。}{Abreast figures. Suitable where no need to write a note under each figure.}\label{fig:abreast-image}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\bisubcaptionbox{这是一张图片。\label{subfig:abreast-image-a}}{Subfigure A.}
	{\includegraphics[height = 5cm]{image-a.pdf}}
	\hspace{1cm}
	\bisubcaptionbox{这又是一张图片。\label{subfig:abreast-image-c}}{Subfigure C.}{\includegraphics[angle = 90, height = 5cm]{image-c.pdf}}
	\bicaption{并排的插图，这适合为每一张图写一个图注的情况。}{Abreast figures. Suitable  where required to write a note under each figure.}\label{fig:abreast-image-a-c}
\end{figure}
...
\subsection{表格}\esubsection{Tables}\label{subsec:tab}
论文中常用三线表。本模板的组成见表~\ref{tab:mainfile}。

\begin{table}[ht]
	\centering
	\bicaption{模板的组成}{The composition of BUCTthesis}\label{tab:mainfile}
		\begin{tabular}{ll}
			\toprule
			文件（夹）名           & 简述\\
			\midrule
...
\begin{table}[H]
	\centering
	\bicaption{表格命令举例}{Examples of the commands in \env{tabular} environment}\label{tab:tabcmd}
...
\begin{table}[H]
	\centering
	\bicaption{这是一个并列排版的示例}{An example of abreast tables}
	\label{tab:2tab}
	\begin{tabular}{|c|r|r|}
		\hline
...
\begin{table}[htbp]
	...
	\begin{minipage}{0.9\textwidth}
		\bicaption{表格控制列宽及自动折行。有些时候标题会比较长，那么我们可以把表格放到一个小页环境里，从而达到比较好的折行效果。}{This is a very very very very very very very long long long long long long long long title.}
		\label{tab:tabularx}
